...
parser.add_argument(
    '-mode', help='Storage mode, FP32 (Default) or FP16', type=str, default='fp32')
...
def get_model_parameters(m):
    ...
    name_list = []
    for k in m['model']:
        if 'encoder.history.weight' in k:
            l = m['model'][k].shape[0]
            for i in range(l):
                name_list.append(k+'{}'.format(i))
                p.append(m['model'][k][i][:i+1])
        elif 'decoder.history.weight' in k:
            l = m['model'][k].shape[0]
            for i in range(l):
                name_list.append(k+'{}'.format(i))
                p.append(m['model'][k][i][:i+1])
        elif 'encoder.embed_tokens.weight' in k:
...
with torch.no_grad():
    ...
    for index, model_file in enumerate(model_files):
        ...
        print('-' * 120)
        print("source model: \'{}\' ({}/{})".format(model_file,
                                                    index+1, len(model_files)))
