                          ...
                          XTensor& padding, IntList* output)
{
    ...
    encoding = model->encoder->Make(input, &maskEnc, false, useFbgemm);
    ...
    /* generate the sequence from left to right */
    int l = 0;
    double decoderTotalT =0;
    double outputTotalT = 0;
    double backStartT = GetClockSec();
    for (; l < maxLength; l++) {
        XTensor prob;
        XTensor maskDec;
        XTensor maskEncDec;
    ...
    double decoderStartT = GetClockSec();
        decoding = model->decoder->Make(inputDec, encoding, NULL, &maskEncDec, l, false, useFbgemm);
	decoderTotalT += (GetClockSec()-decoderStartT);

        /* generate the output probabilities */
    double outputStartT = GetClockSec();
        model->outputLayer->Make(decoding, prob, false, false);
	outputTotalT += (GetClockSec()- outputStartT);
    ...
    LOG("decoder total, elapsed=%.1fs ",decoderTotalT);
    LOG("output total, elapsed=%.1fs ",outputTotalT);
    LOG("backEnd total, %.1fs",GetClockSec()-backStartT);
